...
export function BlogPreview({ posts = [] }) {
  ...
  useGSAP(() => {
    ...
    if (blogPosts.length > 0) {
      gsap.set(blogPosts, { opacity: 0, y: 40 });
  ...
  if (!displayPosts || displayPosts.length === 0) {
    return (
      <section ref={sectionRef} data-blog-section className="relative py-24 px-6 bg-black text-white overflow-hidden z-[60]" suppressHydrationWarning>
  ...
  return (
    <section
      ref={sectionRef}
      data-blog-section
      className="relative py-24 bg-black text-white overflow-hidden z-60"
    ...
    >
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* Stacked Card Grid — 1 col mobile, 2 col sm, 3 col desktop */}
            ...
            return (
              ...
              >
                <article className="h-full flex flex-col border border-white/10 hover:border-white/30 transition-colors duration-300 bg-black/80 backdrop-blur-sm rounded-xl overflow-hidden">
                  {featuredImage && (
                    <div className="relative w-full h-48 md:h-52 shrink-0 overflow-hidden bg-white/5">
                      <Image
                        src={featuredImage}
                        alt={title}
                        fill
      ...
      <div className="mt-16 w-full overflow-hidden border-t border-white/10 bg-black py-4">
